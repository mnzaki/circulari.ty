...
export class ScrimHeddles {
  ...
  /**
   * Create a Divination for async, multi-round validation.
   * 
   * This is the async counterpart to enrich(). Instead of validating
   * synchronously, it returns a Divination that resolves over multiple rounds:
   * 
   ...
   * 
   * Usage:
   *   const divination = heddles.createDivination(management, { lang: typescript });
   *   const result = await divination.resolve(); // or use .watch() for progress
   */
  createDivination(
    mgmt: ScrimManagement,
    options: {
      lang: any;  // LanguageDefinitionImperative
      tags?: string[];
    }
  ): any {  // Divination<ScrimManagement> - imported dynamically to avoid circular deps
    // Dynamic import to avoid circular dependency
    const { createManagementDivination } = require('../divination/heddles-integration.js');
    return createManagementDivination(mgmt, options);
  }
